...
34. ✅ **Transient feedback behavior (2026-09-14, US-005 / FR-3):** Automatically disappearing Snackbars have no close icon. Passive confirmations use 2.5 seconds; feedback with one action uses 4 seconds and remains available after unrelated taps. Swipe, action, timeout, a replacement Snackbar, or any subsequent route push/pop/replace/remove dismisses it. Interface-language success copy, language name, and Undo label are built from the successfully saved target locale, not the departing route context. Current launch-locale text must fit without clipping or avoidable action stacking; accessibility text may grow naturally. The broader visual feedback/offline redesign remains separate. — locked by `2026-09-14-transient-feedback-behavior-design.md`
