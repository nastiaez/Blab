...
35. ✅ **Visible blocked-chat recovery (2026-09-15, Step 3.6a):** Chats renders the complete conversation list so a blocked conversation remains the recovery surface. Immediate-send pickers filter blocked partners separately. Block requires a localized confirmation dialog, then replaces the blocker’s composer with a persistent `You blocked Name · Unblock` row; no redundant Snackbar or blocked-people settings page is added. Message and person reports use the shared passive-success pill. `is_blocked_in_chat` treats either direction as blocking, so the message-insert policy denies both participants until the blocker deletes the row. Invite claims continue to reuse the canonical pair chat and never remove blocks. — locked by `2026-09-15-moderation-feedback-design.md`
