-- K01: resolve source meaning before candidate generation and keep it immutable
-- throughout the semantic audit.
...
begin
  ...
  if p_cache_contract_version <> 'source-meaning-anchor-v8' then
    return false;
  end if;
  perform set_config(
...
select
  message_id, chat_id, viewer_id, learning_language,
  primary_known_language, language_revision, source_version,
  'source_meaning_anchor_cache_invalidated'
...
alter table public.message_translations
  ...
  check (cache_contract_version = 'source-meaning-anchor-v8');
