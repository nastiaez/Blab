-- Apply same-language correction to every eligible learner, including the
-- recipient. Learning aids are shared by message/target/interface locale.

-- Private correction/none analyses encode the superseded author-only rule.
-- Remove them so the next request creates one shared authoritative result.
delete from public.message_translations
where aid_mode in ('correction', 'none');
