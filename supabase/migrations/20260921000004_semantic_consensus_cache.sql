-- K01: replace translations approved by a single semantic review.
...
begin
  ...
  if p_cache_contract_version <> 'semantic-consensus-v6' then
    return false;
  end if;
  perform set_config(
...
select
  message_id, chat_id, viewer_id, learning_language,
  primary_known_language, language_revision, source_version,
  'semantic_consensus_cache_invalidated'
...
alter table public.message_translations
  ...
  check (cache_contract_version = 'semantic-consensus-v6');
