-- K01: independently verify the Known Language sentence and word glosses
-- against the candidate-blind source meaning before saving them.
...
begin
  ...
  if p_cache_contract_version <> 'interface-meaning-audit-v9' then
    return false;
  end if;
  perform set_config(
...
select
  message_id, chat_id, viewer_id, learning_language,
  primary_known_language, language_revision, source_version,
  'interface_meaning_audit_cache_invalidated'
