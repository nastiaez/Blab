-- L11: regenerate translations under the meaning-first false-friend safeguard.
...
begin
  ...
  if p_cache_contract_version <> 'semantic-fidelity-v5' then
    return false;
  end if;
  perform set_config(
...
select
  message_id, chat_id, viewer_id, learning_language,
  primary_known_language, language_revision, source_version,
  'semantic_fidelity_cache_invalidated'
...
alter table public.message_translations
  ...
  check (cache_contract_version = 'semantic-fidelity-v5');
