-- K01: regenerate semantic audits using one stable internal meaning language.
...
begin
  ...
  if p_cache_contract_version <> 'semantic-audit-pivot-v7' then
    return false;
  end if;
  perform set_config(
...
select
  message_id, chat_id, viewer_id, learning_language,
  primary_known_language, language_revision, source_version,
  'semantic_audit_pivot_cache_invalidated'
...
alter table public.message_translations
  ...
  check (cache_contract_version = 'semantic-audit-pivot-v7');
