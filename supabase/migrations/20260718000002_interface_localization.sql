-- L-15: persist a validated four-locale interface preference and isolate
-- translation subtitles/glosses by that interface locale.

update public.profiles
set interface_language = 'en'
where interface_language not in ('en', 'uk', 'de', 'es');

alter table public.profiles
  add constraint profiles_interface_language_check
  check (interface_language in ('en', 'uk', 'de', 'es'));

-- Authored messages may use any source language. Known learning-language
-- codes stay explicit; every other detected source is stored as `other`.
alter table public.message_translations
  drop constraint message_translations_source_lang_check,
...
create or replace function public.update_my_interface_language(
...
begin
  ...
  if p_interface_language not in ('en', 'uk', 'de', 'es') then
    raise exception 'invalid_interface_language' using errcode = '23514';
  end if;

  update public.profiles
  set interface_language = p_interface_language
  where id = v_uid;

  if not found then
    raise exception 'profile_not_found';
  end if;
  return p_interface_language;
end;
$$;

revoke all on function public.update_my_interface_language(text) from public;
grant execute on function public.update_my_interface_language(text)
  to authenticated;

-- New email accounts inherit the locale selected before sign-up. OAuth
-- accounts safely default to English when provider metadata has no locale.
...
begin
  ...
  v_interface_language := case
    when new.raw_user_meta_data ->> 'interface_language'
      in ('en', 'uk', 'de', 'es')
    then new.raw_user_meta_data ->> 'interface_language'
    else 'en'
  end;

  insert into public.profiles (id, display_name, interface_language)
  values (new.id, v_name, v_interface_language);
  return new;
end;
$$;

-- Authored text remains in messages.body and is never replaced by a full
-- interface-language rewrite. Only the learning translation and localized
-- token glosses are cached.
alter table public.message_translations
  drop column english_text;

alter table public.message_translations
  add column interface_lang text not null default 'en',
  add constraint message_translations_interface_lang_check
    check (interface_lang in ('en', 'uk', 'de', 'es'));

alter table public.message_translations
  drop constraint message_translations_pkey,
  add primary key (message_id, target_lang, interface_lang);

create index message_translations_locale_lookup
  on public.message_translations (message_id, target_lang, interface_lang);
...
begin
  ...
  if p_target_lang not in (
    ...
    'nl', 'en', 'fr', 'de', 'hi', 'it', 'pt', 'es', 'ta', 'tr', 'uk',
    'other'
  ) or p_interface_lang not in ('en', 'uk', 'de', 'es') then
    return false;
  end if;
