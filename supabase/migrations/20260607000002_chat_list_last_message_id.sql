-- Expose the last message id on the chat_list view so the chat list
-- tile can fetch / cache that message's translation and render the
-- preview in the viewer's learning language. Appended as the LAST
-- column so the existing CREATE OR REPLACE VIEW path doesn't trip
-- Postgres' "cannot change name of view column" rule.
