-- Server contract for US-042 / FR-34 / FR-35. Form alternatives are shared
-- sentence data; each reader resolves them using their private preferences.
alter table public.message_translations
  add column if not exists form_alternatives jsonb;
...
begin
  ...
  select m.chat_id, m.sender_id, m.body, m.created_at, m.deleted_at,
         cm.learning_language, cm.translation_cutoff_at, cm.mode,
    ...
    into v_chat_id, v_sender_id, v_body, v_created_at, v_deleted_at,
         v_learning_lang, v_cutoff_at, v_mode, v_partner_form, v_tone,
  ...
  from (
    ...
    where m.chat_id = v_chat_id and m.deleted_at is null and m.created_at < v_created_at
      ...
      and (v_cutoff_at is null or m.created_at >= v_cutoff_at)
  ...
  return jsonb_build_object(
    ...
    'targetLang', v_target_lang, 'interfaceLang', v_interface_lang, 'sourceHash', v_source_hash
  );
end;
$$;
...
begin
  ...
  select m.body into v_body from public.messages m join public.chat_members cm on cm.chat_id=m.chat_id
    ...
    join public.profiles p on p.id=p_requester_id and p.interface_language=p_interface_lang
