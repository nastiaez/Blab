-- L-13: make message translation server-authoritative, quota-limited, and
-- resistant to client cache poisoning and edit races.

delete from public.message_translations;

alter table public.message_translations
  add column source_hash text not null,
  add constraint message_translations_source_hash_check
    check (source_hash ~ '^[0-9a-f]{64}$');

drop policy if exists message_translations_insert
...
create table public.translation_usage (
  ...
  day_characters integer not null default 0 check (day_characters >= 0),
  updated_at timestamptz not null default now()
);

alter table public.translation_usage enable row level security;
revoke all on table public.translation_usage
