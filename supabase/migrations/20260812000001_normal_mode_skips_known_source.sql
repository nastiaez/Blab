-- Normal mode stops translating what the reader can already read.
-- See docs/superpowers/specs/2026-08-11-mode-display-fixes-design.md § 3.
--
-- Any prior resolution of a message records its detected `source_lang` in
-- `message_translations`, independent of who asked or which target language
-- they asked for. When the caller is in normal mode and that language is
-- already in their `known_languages`, no learning aid is needed: return the
-- authored text with aid mode `none` and never reach the provider.
...
begin
  ...
  -- Normal mode + source language already known to this reader: no aid.
  -- Cheaper than the cache branch below and hits for every target/interface
  -- combination, not just the caller's own.
  if v_mode = 'normal' and btrim(v_body) <> '' then
    ...
    if v_known_source is not null then
      return jsonb_build_object(
        'status', 'no_aid_needed',
        'mode', 'none',
        'translation', btrim(v_body),
        'interfaceText', btrim(v_body),
        'sourceLang', v_known_source,
