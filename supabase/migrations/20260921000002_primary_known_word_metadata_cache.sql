-- L08: reject copied Learning Language token glosses and regenerate cached
-- packages under the stronger Primary Known Language metadata contract.
...
begin
  ...
  if p_cache_contract_version <> 'primary-known-word-metadata-v4' then
    return false;
  end if;
  perform set_config(
...
-- makes old workers fail by job ID even if they finish during this migration.
...
select
  message_id, chat_id, viewer_id, learning_language,
  primary_known_language, language_revision, source_version,
  'primary_known_word_metadata_cache_invalidated'
