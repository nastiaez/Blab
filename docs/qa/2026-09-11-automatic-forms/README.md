...
- The first fresh-database CI run exposed stale integration setup from the connection-only invite redesign. Tests were still passing ignored invite-language arguments, leaving new members unconfigured and translation requests correctly forbidden. The fixtures now select each participant's language explicitly, verify that re-invites preserve those choices, and call the service-role-only v2 cache completion overload used by the Edge Function.
